$( function() {
    ...
    $(".center-body-left li").on('click',function () {
        var div = $(".center-body-left li");
        var linum = div.index(this);
    ...
    function checkAddPhone() {
        var username = $("#addPhone").val();
        if(/^1[34578]\d{9}$/.test(username)){
            clearAddPhoneTip();
        }
        else{
            $("#addPhoneTips").css('color','red').html('请填写正确的手机号') ;
        }

    }

    function checkAddPhoneVercode() {
        var username = $("#addPhoneVercode").val();
        if(username !== ''){
            clearAddPhoneTip();
            checkAddPhone()
        }
        else{
            $("#addPhoneTips").css('color','red').html('请输入验证码') ;
        }
    }

    var clearAddPhoneTip = function () {
        var tip = $("#addPhoneTips");
        if (tip.html() !== '') tip.html('');
    };


    $("#addPhone").on('change',checkAddPhone);
    $("#addPhoneVercode").on('change',checkAddPhoneVercode);

    var d = 60;
    function a() {
        var e = $("#addGetVercode");
        if (0 === d) return e.attr("disabled", !1), e.val("点击获取"), void(d = 60);
        e.attr("disabled", !0), e.val("刷新(" + d + ")"), d--, setTimeout(function() {
            a()
        }, 1e3)
    };
    $("#addGetVercode").on('click',function () {
        checkAddPhone();
        if($("#addPhoneTips").html() === ''){
            $.ajax({
                dataType:'text',
                url:'/sendMsg',
    ...
    $('#submitPhone').on('click',function () {
        checkAddPhoneVercode();
        if($("#addPhoneTips").html() === ''){
            $.ajax({
                dataType:'text',
                async:!1,
                url:'/checkCode',
                data:{'code':$('#addPhoneVercode').val()},
                success:function (rst) {
                    if(rst === 'error'){
                        $("#addPhoneTips").css('color','red').html('验证码输入错误') ;
                    }
                    if(rst === 'success'){
                        $.ajax({
                            type:'GET',
                            ...
                            data:{'phone':$("#addPhone").val()},
                            success:function (rst) {
                                if(rst === 'input') $("#addPhoneTips").css('color','red').html('手机号已被绑定') ;
                                if (rst=== 'success') {
                                    window.location.href='/userInfo';
                                    window.wxc.xcConfirm('您已成功绑定手机号'+$("#addPhone").val(), window.wxc.xcConfirm.typeEnum.info);
                                }
                            }
                        })
                    }
